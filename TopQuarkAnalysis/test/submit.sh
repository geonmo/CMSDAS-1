...
for D in DoubleEG_Run2015C DoubleEG_Run2015D DoubleMuon_Run2015C DoubleMuon_Run2015D MuonEG_Run2015C MuonEG_Run2015D; do
  echo $D
  $CMD --jobName $D --fileList dataset/dataset_$D.txt --transferDest $OUTBASE/$D
done
...
for D in 15to20 20to30 30to50 50to80 80to120 120to170 170to300; do
  echo $D
  $CMD --jobName QCD_Pt-${D}_MuEnriched --fileList dataset/dataset_QCD_Pt-${D}_MuEnriched.txt --transferDest $OUTBASE/QCD_Pt-${D}_MuEnriched
  $CMD --jobName QCD_Pt-${D}_EMEnriched --fileList dataset/dataset_QCD_Pt-${D}_EMEnriched.txt --transferDest $OUTBASE/QCD_Pt-${D}_EMEnriched
done
...
for D in TT_powheg TTJets_MG5 TTJets_aMC TT_amcatnlo-herwigpp TT_powheg-herwigpp TT_powheg_pythia6; do
  echo $D
  $CMD --jobName ${D}_LL --fileList dataset/dataset_$D.txt --transferDest $OUTBASE/$D
  $CMD --jobName ${D}_Others --fileList dataset/dataset_$D.txt --transferDest $OUTBASE/$D --args "filterPartonTTLL.invert=True"
done
